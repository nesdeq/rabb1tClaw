...
pub const DEFAULT_MIN_CONTEXT_MESSAGES: usize = 3;
...
pub const DEFAULT_CODE_MAX_STATUS_TOKENS: usize = 256;
pub const DEFAULT_CODE_EXEC_TIMEOUT_SECS: u64 = 120;
pub const DEFAULT_CODE_PRUNE_AGE_SECS: u64 = 3600;
...
pub const DEFAULT_SEARCH_MAX_DEEP_READ_URLS: usize = 5;
pub const DEFAULT_SEARCH_MAX_PAGE_TOKENS: usize = 2500;
pub const DEFAULT_SEARCH_FETCH_TIMEOUT_SECS: u64 = 15;
pub const DEFAULT_SEARCH_PRUNE_AGE_SECS: u64 = 3600;
...
pub(crate) fn apply_smart_defaults(
    ...
    match detect_tier(api_type, &mc.model_id) {
        ModelTier::Reasoning => {
            ...
            println!("  Auto: reasoning model — reasoning_effort: {}, max_tokens: {}", DEFAULT_REASONING_EFFORT_OPENAI, DEFAULT_REASONING_MAX_TOKENS_OPENAI);
        }
        ModelTier::Thinking => {
            ...
            println!("  Auto: thinking model — thinking: on (budget: {}), max_tokens: {}", DEFAULT_THINKING_BUDGET_TOKENS_ANTHROPIC, DEFAULT_THINKING_MAX_TOKENS_ANTHROPIC);
        }
...
pub(crate) fn populate_default_agents(config: &mut crate::config::GatewayConfig) {
    ...
    if main.min_context_messages.is_none() { main.min_context_messages = Some(DEFAULT_MIN_CONTEXT_MESSAGES); }
    ...
    if code.max_status_tokens.is_none()  { code.max_status_tokens = Some(DEFAULT_CODE_MAX_STATUS_TOKENS); }
    if code.exec_timeout_secs.is_none()  { code.exec_timeout_secs = Some(DEFAULT_CODE_EXEC_TIMEOUT_SECS); }
    if code.prune_age_secs.is_none()     { code.prune_age_secs = Some(DEFAULT_CODE_PRUNE_AGE_SECS); }
    ...
    if search.max_deep_read_urls.is_none()   { search.max_deep_read_urls = Some(DEFAULT_SEARCH_MAX_DEEP_READ_URLS); }
    ...
    if search.prune_age_secs.is_none()       { search.prune_age_secs = Some(DEFAULT_SEARCH_PRUNE_AGE_SECS); }
}
