...
fn build_container(workspace: &Path, python_prefix: &Path, network: bool) -> anyhow::Result<hakoniwa::Container> {
...
pub(super) fn ensure_venv(workspace: &Path, exec_timeout_secs: u64) -> anyhow::Result<PathBuf> {
    ...
    info!("Created venv at {}", venv.display());
    Ok(prefix)
}

/// Install packages via pip in the sandbox. Returns (success, combined_output).
/// Always uses the workspace venv's pip.
pub(super) fn pip_install(workspace: &Path, python_prefix: &Path, packages: &[String], exec_timeout_secs: u64) -> anyhow::Result<(bool, String)> {
...
pub(super) fn execute_in_sandbox(
    workspace: &Path,
    python_prefix: &Path,
    script_name: &str,
